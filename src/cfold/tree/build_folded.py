...
def build_folded_tree(files: List[Path], cwd: Path) -> Tree:
    """Generate a Rich Tree object for the folded files with dim styling, percentages, and heat colors."""
    ...
    main_tree = Tree(
        f"Folded files tree (total lines: {total_lines})", guide_style="grey50"
    )
    for file_path_rel in sorted(line_counts.keys()):
        ...
        for i, part in enumerate(parts[:-1]):
            ...
            subnodes = [
                ...
                if node.label.startswith(f"[yellow]{part}/")
            ]
            if subnodes:
                ...
                if dir_path in dir_totals:
                    ...
                    label = f"[yellow]{part}/[/yellow] ({dir_lines} lines, [{color}]{percent:.1f}%[/{color}])"
