    ...
    try:
        ...
        if common_system:
            instructions_list.append(
                Instruction(type="system", content=common_system, name="common")
            )
        dialect_system = instr.get("system", "")
        if dialect_system:
            instructions_list.append(
                Instruction(type="system", content=dialect_system, name=dialect)
            )
        dialect_user = instr.get("user", "")
        if dialect_user:
            instructions_list.append(
                Instruction(type="user", content=dialect_user, name=dialect)
            )
        dialect_assistant = instr.get("assistant", "")
        if dialect_assistant:
            instructions_list.append(
                Instruction(type="assistant", content=dialect_assistant, name=dialect)
            )
